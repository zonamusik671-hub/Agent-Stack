...
import { AnchorClient, deriveVaultAuthority, deriveVaultTokenAccount, } from './anchorClient.js';
import { fetchLivePreStocksAssets } from './prestocksApi.js';
import { AAPL_FEED_ID, getLatestPythPrice } from './pythConnection.js';
import { createClawpumpFundingPlan, createClawpumpLaunchPlan, executeClawpumpFunding, executeClawpumpLaunch, persistClawpumpLaunchProof, } from './clawpump.js';
...
function getOrchestratorLockPath(vaultAccountAddress, idBasket) {
    const safeVault = vaultAccountAddress.replace(/[^A-Za-z0-9_-]/g, '_');
    return join(process.env.ASB_LOCK_DIRECTORY?.trim() || tmpdir(), `asb-orchestrator-${safeVault}-${idBasket}.lock`);
}
async function acquireDistributedLock(lockPath) {
    try {
...
async function retryCycle(operation, attempts = 3) {
    let lastError;
...
export function shouldRebalance({ liveStockPrice, threshold, }) {
...
function loadAgentKeypair() {
    const keypairPath = process.env.AGENT_KEYPAIR_PATH ?? `${homedir()}/.config/solana/id.json`;
    try {
...
function getClawpumpFundingAmount() {
    const configured = process.env.CLAWPUMP_FUNDING_LAMPORTS?.trim();
    if (!configured)
        return DEFAULT_CLAWPUMP_FUNDING_LAMPORTS;
    try {
...
async function executeClawpumpAfterRebalance(agentKeypair, basketId) {
    ...
    const options = { mode: mode, connection, payer: agentKeypair };
...
export async function runAgentOrchestrator(vaultAccountAddress, idBasket) {
    console.log(`[ASB Orchestrator] Menjalankan Agen AI untuk Vault: ${vaultAccountAddress} (basket ${idBasket})`);
    const runCycle = async () => {
        const releaseLock = await acquireDistributedLock(getOrchestratorLockPath(vaultAccountAddress, idBasket));
        if (!releaseLock) {
        ...
        try {
            ...
            const preStocksAssets = await retryCycle(() => fetchLivePreStocksAssets());
            const targetAsset = preStocksAssets.find((asset) => asset.symbol === 'AAPL' || asset.symbol === 'AAPLX');
            ...
            if (shouldRebalance({ liveStockPrice, threshold: tradingThreshold })) {
                ...
                const rebalanceSignature = await sendOnChainRebalance(vaultPubkey, rwaMintPubkey, amountToRebalance, tokenDecimals, idBasket);
...
async function sendOnChainRebalance(vaultPubkey, rwaMintPubkey, amount, decimals, expectedBasketId) {
    if (amount <= 0n) {
    ...
    const destinationTokenAccount = getAssociatedTokenAddressSync(rwaMintPubkey, agentKeypair.publicKey, false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
    try {
        const transaction = await client.program.methods
            .executeRebalance(new anchor.BN(amount.toString()), new anchor.BN(expectedBasketId))
            .accounts({
        ...
        transaction.partialSign(agentKeypair);
        const signature = await sendAndConfirmRawTransaction(connection, transaction.serialize(), { commitment: 'confirmed' });
