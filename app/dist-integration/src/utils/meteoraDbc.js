...
export function getMeteoraSwapTransactionMetadata(transaction) {
    return transaction[METEORA_SWAP_METADATA];
}
export function setMeteoraSwapTransactionMetadata(transaction, metadata) {
    transaction[METEORA_SWAP_METADATA] = metadata;
}
...
export function validateLivePreStocksMint(asset) {
...
export function createMeteoraDbcClient(endpoint = METEORA_DBC_RPC_ENDPOINT) {
...
export async function quoteMeteoraSwap(params) {
    ...
    const poolConfigAddress = virtualPool.config;
 ...
 */
export async function prepareMeteoraSwapTest(params) {
...
export async function executeMeteoraSwap(params) {
    ...
    try {
        return await sendAndConfirmRawTransaction(connection, transaction.serialize(), { commitment: 'confirmed' });
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
