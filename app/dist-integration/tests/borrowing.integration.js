...
import { AnchorClient, deriveVault, deriveVaultAuthority, deriveVaultTokenAccount, } from '../src/utils/anchorClient.js';
class KeypairWallet {
    payer;
    constructor(payer) {
        this.payer = payer;
...
function liquidationCollateral(collateral, debt, repayment, bonusBps) {
    if (debt <= 0n || repayment <= 0n || repayment > debt) {
        throw new Error('invalid repayment');
    }
    const base = (collateral * repayment) / debt;
    return ((base * (10000n + bonusBps)) / 10000n) > collateral
        ? collateral
        : (base * (10000n + bonusBps)) / 10000n;
}
test('liquidation bonus is bounded and cannot release more collateral than locked', () => {
    assert.equal(liquidationCollateral(1000n, 500n, 100n, 500n), 210n);
    assert.equal(liquidationCollateral(100n, 1n, 1n, 10000n), 100n);
});
...
test('Borrow -> oracle price drop -> liquidation against a mock oracle account', {
    ...
    if (phase === 'borrow') {
        ...
        await mintTo(connection, owner, vaultMint, collateralAccount, owner, 1000000n, [], undefined, TOKEN_2022_PROGRAM_ID);
        ...
        await mintTo(connection, owner, loanMint, loanTokenAccount, owner, 1000000n, [], undefined, TOKEN_2022_PROGRAM_ID);
    ...
    await mintTo(connection, owner, loanMint, liquidatorLoanAccount, owner, 250000n, [], undefined, TOKEN_2022_PROGRAM_ID);
    ...
    assert.equal(after - before, 525000n);
