...
const DEFAULT_AAPL_FEED_ID =
  'f9c017263a506240646467c9d2833076137d076d000000000000000000000001';

export type PythCluster = 'devnet' | 'mainnet-beta';

export interface PythOnChainConfig {
  cluster: PythCluster;
  programId: PublicKey;
  priceFeed: PublicKey;
  feedId: string;
...
export function getPythOnChainConfig(
  cluster: PythCluster = process.env.SOLANA_CLUSTER === 'mainnet-beta'
    ? 'mainnet-beta'
    : 'devnet',
): PythOnChainConfig {
...
function getPythHermesEndpoint(): string {
  return process.env.PYTH_HERMES_ENDPOINT?.trim() || DEFAULT_PYTH_HERMES_ENDPOINT;
}

function getPythAccessToken(): string | undefined {
...
export function hasConfiguredPythHermesToken(): boolean {
  return getPythAccessToken() !== undefined;
}

export async function validatePythHermesToken(
...
export const AAPL_FEED_ID = (
  process.env.PYTH_FEED_ID ?? DEFAULT_AAPL_FEED_ID
).replace(/^0x/, '');

export async function getLatestPythPrice(feedId: string): Promise<number> {
  ...
  if (typeof window !== 'undefined') {
    ...
    const parsed = (payload as { parsed?: Array<{ price?: { price?: string; expo?: number } }> }).parsed;
...
export function formatPythPrice(price: bigint, expo: number | bigint): string {
...
export function parsePythPrice(price: bigint, expo: number | bigint): number {
