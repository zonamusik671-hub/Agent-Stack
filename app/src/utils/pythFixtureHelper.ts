...
export interface PythFixtureValues {
  price: bigint;
  confidence: bigint;
  exponent: number;
  timestamp: bigint;
...
export interface PythFixtureAccount {
  address: PublicKey;
  owner: PublicKey;
  lamports: number;
  data: Buffer;
...
function writeI64(data: Buffer, offset: number, value: bigint): void {
 ...
 */
export function createPythPriceAccountFixture(values: PythFixtureValues): Buffer {
  if (values.price <= 0n) throw new RangeError('Pyth price must be positive');
  if (values.confidence < 0n) throw new RangeError('Pyth confidence cannot be negative');
...
export function validatePythPriceAccountFixture(data: Uint8Array): Buffer {
...
export async function fetchPythPriceAccountFixture(
  connection: Connection,
  address: PublicKey,
  expectedOwner: PublicKey,
  outputPath: string,
): Promise<PythFixtureAccount> {
  const account = await connection.getAccountInfo(address, 'confirmed');
  if (!account) throw new Error(`Pyth account not found: ${address.toBase58()}`);
...
export async function readPythPriceAccountFixture(path: string): Promise<Buffer> {
  return validatePythPriceAccountFixture(await readFile(path));
}

export async function writeValidatorAccountFixture(
  path: string,
  account: Pick<PythFixtureAccount, 'owner' | 'lamports'> & { data: Uint8Array },
): Promise<void> {
  const data = validatePythPriceAccountFixture(account.data);
