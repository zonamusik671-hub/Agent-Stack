...
test(
  'rejects an execute_rebalance signed by an unauthorized agent',
  ...
  async () => {
    ...
    const unauthorizedClient = new AnchorClient(
      connection,
      new KeypairWallet(unauthorized),
    );
    await assert.rejects(
      unauthorizedClient.executeRebalance({
        vault,
        vaultTokenAccount,
      ...
      (error: unknown) =>
        String(error).includes('UnauthorizedAgent') ||
...
test(
  'deposits Token-2022 USDC and executes an authorized rebalance',
  { skip: process.env.RUN_ANCHOR_INTEGRATION !== '1' },
  async () => {
    const endpoint = process.env.ANCHOR_PROVIDER_URL ?? 'http://127.0.0.1:8899';
    const connection = new Connection(endpoint, 'confirmed');
    ...
    const rebalanceAmount = 400_000n;
    ...
    const vaultBalanceAfterDeposit = await getAccount(
      connection,
      vaultTokenAccount,
      'confirmed',
      TOKEN_2022_PROGRAM_ID,
    ...
    const vaultBalanceAfterRebalance = await getAccount(
      connection,
      vaultTokenAccount,
      'confirmed',
      TOKEN_2022_PROGRAM_ID,
