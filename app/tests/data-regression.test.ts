...
import {
  getPreStocksApiBaseUrl,
  mapLivePreStocksAsset,
  type PreStocksApiAsset,
} from '../src/utils/prestocksApi.js';
import {
  formatPythPrice,
  hasConfiguredPythHermesToken,
  parsePythPrice,
  validatePythHermesToken,
} from '../src/utils/pythConnection.js';
import { shouldRebalance } from '../src/utils/agent-orchestrator.js';
import {
  MOCK_PYTH_TOKEN,
  startMockSponsorServer,
} from './mock-sponsor-server.js';
...
test('routes Pyth and PreStocks through the isolated mock sponsor server', async () => {
  ...
  try {
    ...
    if (originalPythEndpoint === undefined) delete process.env.PYTH_HERMES_ENDPOINT;
    else process.env.PYTH_HERMES_ENDPOINT = originalPythEndpoint;
    if (originalPreStocksEndpoint === undefined) delete process.env.PRESTOCKS_API_BASE_URL;
    else process.env.PRESTOCKS_API_BASE_URL = originalPreStocksEndpoint;
    if (originalToken === undefined) delete process.env.PYTH_PRO_TOKEN;
    else process.env.PYTH_PRO_TOKEN = originalToken;
  }
});
...
test('rejects invalid rebalance decision inputs', () => {
  assert.throws(
    () => shouldRebalance({ liveStockPrice: Number.NaN, threshold: 180 }),
    /finite positive/,
  );
  assert.throws(
    () => shouldRebalance({ liveStockPrice: 100, threshold: 0 }),
    /finite positive/,
  );
});
...
test('uses the configured PreStocks API base URL without changing validation rules', () => {
  ...
  try {
    ...
    if (originalBaseUrl === undefined) delete process.env.PRESTOCKS_API_BASE_URL;
    else process.env.PRESTOCKS_API_BASE_URL = originalBaseUrl;
  }
});
...
test('resolves only when Hermes responds with HTTP 200', async () => {
  ...
  try {
    ...
    await assert.rejects(
      validatePythHermesToken(async () => new Response('{}', { status: 401 })),
      /HTTP 401/,
    );
  } finally {
    if (originalToken === undefined) delete process.env.PYTH_PRO_TOKEN;
