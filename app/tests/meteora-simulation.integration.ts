...
import {
  configureMeteoraPool,
  getMeteoraSwapTransactionMetadata,
  setMeteoraSwapTransactionMetadata,
  simulateMeteoraSwapTransaction,
  validateLivePreStocksMint,
} from '../src/utils/meteoraDbc.js';
...
test('simulates a registered Meteora swap transaction with slippage protection', {
  ...
  const minimumAmountOut = 950_000n;
  ...
  assert.equal(getMeteoraSwapTransactionMetadata(transaction)?.minimumAmountOut, 950_000n);
